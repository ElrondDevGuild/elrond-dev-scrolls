---
title: Issue ESDT - example of setup and interaction using the JS SDK
description: Setup the JS SDK workflow and issue an ESDT token in NodeJS enviroment.
---
...
The eStandard Digital Token is a built-in standard for custom tokens on the MultiversX blockchain. It means anyone can create their token with similar properties to native EGLD.

If you don't know much about ESDT yet, please read about it here: [Introduction to ESDT tokens](https://docs.multiversx.com/tokens/esdt-tokens). You will find a more general description of all transactions. It is essential to know that, but here we will focus more on the [JS SDK](https://docs.multiversx.com/sdk-and-tools/sdk-js/) and how to use it to issue ESDT tokens. So probably a more practical approach.
...
This article will be the first in a series about JS SDK and tokens-related operations. We are starting by issuing the ESDT token.

To start learning and playing around, it is crucial to understand what [JS sdk-core](https://github.com/multiversx/mx-sdk-js-core) is.

It is a set of functions and helpers that simplifies interaction with the MultiversX blockchain and custom smart contracts. The tool is written in Typescript and can be used in NodeJS or browsers.

It is the main package, but there are a couple of complementary libraries. You will find them here: [JS SDK libraries](https://docs.multiversx.com/sdk-and-tools/sdk-js/).
...
Each transaction made with JS SDK (and issuing the ESDT token is such a transaction) requires two things:

1. Wallet to be able to sign transactions
2. Network provider - tools and configuration for the API endpoint

These two we will split into more granular areas. Let's take the wallet first. We already have some information about how to work with the PEM files here [Use PEM file wallet](https://elrond-dev-guild.gitbook.io/scrolls/js-sdk/how-tos/use-pem-file-wallet). You can check this out first.

The PEM file is a file with your private key, so don't share it with anyone. It isn't the most secure way to interact, because of human factor, but it is probably the simplest and fastest. You can also use the wallet JSON file with a password, and You would probably use the Ledger hardware wallet or xPortal mobile app in a browser environment. But for simplicity and because we will work in the Nodejs environment, only locally, let's use a PEM file.

What you would need is to create such a file. You can do this in two ways:

- using the [Python SDK](https://docs.multiversx.com/sdk-and-tools/sdk-py/) (CLI), check how here: [Deriving the PEM file](https://docs.multiversx.com/sdk-and-tools/sdk-py/deriving-the-wallet-pem-file/)
- or you can also use JS SDK based tool: [Buildo Begins](https://github.com/xdevguild/buildo-begins#available-commands-for-buildo-for-now-there-will-be-more)
...
In return, you will get the wallet PEM file. Keep it and remember the path to it. For the wallet part, we have all already. We have the file that will help us sign all transactions. Remember that the PEM file should be derived from a seed phrase from a wallet from the devnet. Also, please fund the wallet using the devnet faucet, which you will find in the [devnet web wallet](https://devnet-wallet.multiversx.com/).

Next, we will need a couple of helpers to simplify interactions with MultiversX API. We could, of course, prepare our tools for that. For example, use Axios to make calls, etc. But there is a ready-to-use library called [@multiversx/sdk-network-providers](https://github.com/multiversx/mx-sdk-js-network-providers). We will use it for simplicity. But feel free to prepare your tools. See how in the docs: [Extending the Network Providers](https://docs.multiversx.com/sdk-and-tools/sdk-js/extending-sdk-js/#extending-the-network-providers).
...
Let's assume that your NodeJS app is already initialized and you installed the two mentioned above libraries, the [@multiversx/sdk-core](https://www.npmjs.com/package/@multiversx/sdk-core) and [@multiversx/sdk-network-providers](https://www.npmjs.com/package/@multiversx/sdk-network-providers). Oh btw, we will also need the third one [@multiversx/sdk-wallet](https://www.npmjs.com/package/@multiversx/sdk-wallet). More about it later in the article.
...
Let's see what it looks like when it comes to code. You will also find the links to the real use case in [Buildo Begins](https://github.com/xdevguild/buildo-begins) library.
...
```
You will find the real example in the Buildo Begins library: [https://github.com/xdevguild/buildo-begins](https://github.com/xdevguild/buildo-begins).

We can then use the file contents across the app.

## Initialize Network Provider

For that we will use [@multiversx/sdk-network-providers](https://github.com/multiversx/mx-sdk-js-network-providers). 
...
```
Here simplified, and you will find the real example in the Buildo Begins library: [https://github.com/xdevguild/buildo-begins](https://github.com/xdevguild/buildo-begins).

The provider will be used wherever we need to interact with the MultiversX API. It can be the public endpoint as in the example, but it is always advised to use a custom one—yours or from third-party services.
...
For that we will need another JS SDK Wallet library: [@multiversx/sdk-wallet](https://www.npmjs.com/package/@multiversx/sdk-wallet).
...
UserSigner is a helper class that allows signing the transactions using the walletKey.pem file. Here we will also need the [@multiversx/sdk-wallet](https://www.npmjs.com/package/@multiversx/sdk-wallet) library.
...
As you can see we use the `UserSigner.fromPem` function to create the signer using our PEM file. It can do the same using the JSON file and password. Check how it looks here: [userSigner.ts](https://github.com/multiversx/mx-sdk-js-wallet/blob/main/src/userSigner.ts).

## Real-life example and next steps

With the JS SDK setup now, we are ready to go. Here you will find fully functional ESDT issue token operations: [issue-esdt.ts](https://github.com/xdevguild/buildo-begins/blob/f661798936be9e5fdc521fbb25cac5c93b57617a/src/esdt/issue-esdt.ts).
...
Each issuance of the ESDT token on the MultiversX blockchain costs 0.05 EGLD. This is a predefined cost. But don't confuse it. This is the cost of issuing, not the fee when you want to buy or sell some amount.

To properly prepare and send the amount, we need to use a `TokenPayment` from JS SDK.
...
The arguments are put into the array of `TypedValue` items. JS SDK provides helper classes to keep strong typings for each type of argument. This is required to prepare the payload for the transactions correctly. So below, you will find an example.
...
For more data types, you can check the JS SDK core source code here: [typesystem](https://github.com/multiversx/mx-sdk-js-core/tree/main/src/smartcontracts/typesystem).
...
```typescript
import { ContractCallPayloadBuilder, ContractFunction } from '@multiversx/sdk-core';
(...)
...
As you can see, we are passing our `args` array here, but we also need to pass the smart contract function to call. Here it is a built-in `issue` function. Again you will find all the information about ESDT in the docs here: [Issuance of fungible ESDT tokens](https://docs.multiversx.com/tokens/esdt-tokens/#issuance-of-fungible-esdt-tokens).

Btw. you can find more helpers for payload creation here: [TransactionPayload](https://github.com/multiversx/mx-sdk-js-core/blob/main/src/transactionPayload.ts) and [Smart contract transaction payload builders](https://github.com/multiversx/mx-sdk-js-core/blob/main/src/smartcontracts/transactionPayloadBuilders.ts).
...
For this, we will use the `TransactionWatcher` from JS SDK. 
...
You can read the status directly. You can also get the hash of the transaction at this stage. Now the whole process should be finished, and you should be able to use the MultiversX explorer to see the entire history using the transaction hash.
...
export const getProvider = () => {
  return new ApiNetworkProvider('https://api.multiversx.com', {
    timeout: 10000,
  });
};
```
...
The flow here is very similar for each transaction. The only differences will be in preparing the data payload, arguments, and sometimes slight differences in transaction instances. All is described in the docs, so you would only need to do the same using helper classes from JS SDK.

Here we issued the ESDT token. Of course, this wasn't a complete application, but there is one, and it is open source, so you can check the code by yourself and even copy what you need. You will find it here: [https://github.com/xdevguild/buildo-begins](https://github.com/xdevguild/buildo-begins), and this particular functionality is here: [issue-esdt.ts](https://github.com/xdevguild/buildo-begins/blob/main/src/esdt/issue-esdt.ts).
...
If you liked the article, there would be more like this one, maybe focused more on token operation instead of the JS SDK required tools. The plan is to describe all that will land in the Buildo Begins library.
